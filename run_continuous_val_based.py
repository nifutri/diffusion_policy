...
human_data_path = f'{home_path}/Code_robocasa/robocasa_dagger/diffusion_policy/data/outputs/ST_OOD_DAgger_train_diffusion_unet_clip_OpenSingleDoor/eval_base_policy/merged_demos_success_im256.hdf5'
...
for i in range(num_epochs_iterative_procedure):
    ...
    cmd = f"CUDA_VISIBLE_DEVICES={cuda_device} python evaluate.py --config-name=eval_robocasa_base_policy_val_fct proposal_policy_ckpt_path={proposal_policy_path} value_policy_ckpt_path={bad_policy_path} num_rollouts={num_rollouts}"
    if initialization:
