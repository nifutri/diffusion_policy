...
class EvalDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        if experiments_data['tasks'][task_name]['environment_file'] in ['demo_gentex_im256_randcams_100_train_envs.pkl', 'demo_gentex_im256_randcams_100_texture_changed_train_envs.pkl']:
            path, ds_meta = get_ds_path(task=task_name, ds_type="human_im", return_info=True)  # human dataset path
        elif experiments_data['tasks'][task_name]['environment_file'] in ['demo_gentex_im128_randcams_100_mg_envs.pkl', 'demo_gentex_im128_randcams_100_position_changed_mg_envs.pkl', 'demo_gentex_im128_randcams_100_object_changed_mg_envs.pkl']:
            path, ds_meta = get_ds_path(task=task_name, ds_type="mg_im", return_info=True)        # MimicGen dataset path

        with open(f"{os.path.dirname(path)}/{experiments_data['tasks'][task_name]['environment_file']}", "rb") as pickle_file:
        ...
        for demo in demos:
            ...
            task_description = json.loads(initial_state['ep_meta'])['lang']
