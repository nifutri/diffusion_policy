...
class MultiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        sample1 = self.dataset1[idx % len(self.dataset1)]
        sample2 = self.dataset2[idx % len(self.dataset2)]
...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        accelerator = Accelerator(log_with='wandb', gradient_accumulation_steps=cfg.training.gradient_accumulate_every, mixed_precision="fp16")
        ...
        # configure dataset
        # GOOD DATASET:
        good_dataset = hydra.utils.instantiate(cfg.task.dataset)
        #make a copy of the dataset params:
        bad_dataset_params = copy.deepcopy(cfg.task.dataset)
        bad_dataset_params.human_path = cfg.bad_human_path
        bad_dataset = hydra.utils.instantiate(bad_dataset_params)
        dataset = MultiDataset(good_dataset, bad_dataset)
        ...
        lr_scheduler = get_scheduler(
            ...
            num_training_steps=(
                len(train_dataloader) * cfg.training.lr_scheduler_epochs) \
        ...
        # eventually compile - but before compilation move to the appropriate device!
        self.model.to(accelerator.device)
        # eventually compile - not done now here since it causes issues with the logging,...
        # self.model = torch.compile(self.model, mode="reduce-overhead")

        # from torch.nn.parallel import DistributedDataParallel as DDP
        # self.model = DDP(self.model, device_ids=[accelerator.device.index], find_unused_parameters=True)
        ...
        if cfg.training.restore_accelerator and os.path.exists(cfg.training.previous_accelerator_path):
            accelerator.load_state(cfg.training.previous_accelerator_path)
            print(f"Restored accelerator state from {cfg.training.previous_accelerator_path}")

        # do ema only after accelerator has been prepared
        self.ema_model: DiffusionUnetTimmPolicy = None
        if cfg.training.use_ema:
            self.ema_model = copy.deepcopy(accelerator.unwrap_model(self.model))
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        # print (batch[0].keys())
                        # print (batch[0]['obs']['left_image'].shape)
                        # print (batch[1].keys())
                        # print (batch[1]['obs']['left_image'].shape)
                        # input ("WAIT")
                        # device transfer
                        batch0 = dict_apply(batch[0], lambda x: x.to(device, non_blocking=True))
                        batch1 = dict_apply(batch[1], lambda x: x.to(device, non_blocking=True))
                        
                        # # always use the latest batch
                        # train_sampling_batch = batch

                        with accelerator.accumulate(self.model):  # <-- key wrapper
                            loss = self.model(batch0,batch1)
                            raw_loss_value = loss.item()
                            # loss = outputs / cfg.training.gradient_accumulate_every
                            accelerator.backward(loss)

                            if accelerator.sync_gradients:
                                accelerator.clip_grad_norm_(self.model.parameters(), max_norm=0.25)
                                self.optimizer.step()
                                lr_scheduler.step()
                        ...
                        # loss.backward()
                        # print (f"batch transfer took {time.time() - start:.6f} seconds")
                        ...
                        #     self.optimizer.zero_grad()
                        #     lr_scheduler.step()
                        #     print (f"optimizer step took {time.time() - start:.6f} seconds")
                        

                        # logging
                        raw_loss_cpu = raw_loss_value
                ...
                # checkpoint
                if ((self.epoch % cfg.training.checkpoint_every) == 0 or (self.epoch == cfg.training.num_epochs-1)) and accelerator.is_main_process:
                    ...
                    checkpoint_filename = self.output_dir + f"/checkpoints/epoch_{self.epoch}.ckpt"
                    self.save_checkpoint(path=checkpoint_filename)
                    # now save the accelerator in case we want to resume training later
                    checkpoint_accelerator = self.output_dir + f"/checkpoints/epoch_{self.epoch}_accelerator"
                    accelerator.save_state(checkpoint_accelerator)
