...
class RobocasaImageWrapper(gym.Env):
    ...
    def get_observation(self, raw_obs=None):
        ...
        for key in self.observation_space.keys():
            if key not in raw_obs:
                assert 'image' in key
                short_key = key.split('_image')[0]
                # call env.sim.render
                obs[key] = self.env.sim.render(camera_name=short_key, width=256, height=256)
            else:
                obs[key] = raw_obs[key]
        return obs
    ...
    def reset(self):
        if self.init_state is not None:
            ...
            # pdb.set_trace()
            raw_obs = reset_to(self.env, self.init_state)
            ...
            #     raw_obs = reset_to(self.env, {'states': self.seed_state_map[seed]})
            # else:
