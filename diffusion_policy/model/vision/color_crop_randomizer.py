...
# from diffusion_policy.model.common.random_crop import sample_random_image_crops


class CropRandomizer(nn.Module):
    """
    Randomly sample crops and apply color perturbation during training.
    Returns center crop during evaluation. Supports multiple crops and optional positional encoding.
    """
    def __init__(
        self,
        input_shape,
    ...
    def forward_in(self, inputs):
        """
        During training: sample N random crops, apply color jitter, and return shape [B * N, ...].
        During eval: return center crop with optional repetition if N > 1.
        """
        assert inputs.ndim == 4  # (B, C, H, W)
        if self.training:
            # Sample random crops
            crops, _ = sample_random_image_crops(
                images=inputs,
            ...
            # [B, N, C, H, W] -> [B * N, C, H, W]
            crops = tu.join_dimensions(crops, 0, 1)

            # Apply color jitter to RGB channels
            rgb = crops[:, :3]
            extra = crops[:, 3:] if self.pos_enc else None

            rgb_aug = torch.stack([
                ttf.to_tensor(self.color_jitter(ttf.to_pil_image(img.cpu()))).to(img.device)
                for img in rgb
            ])

            # Concatenate positional encodings (if any)
            crops_aug = torch.cat([rgb_aug, extra], dim=1) if self.pos_enc else rgb_aug
            return crops_aug

        else:
            # Eval: take center crop
            out = ttf.center_crop(inputs, output_size=(self.crop_height, self.crop_width))
            if self.num_crops > 1:
                B, C, H, W = out.shape
    ...
    def forward_out(self, inputs):
        """
        [B * N, ...] -> [B, ...] by averaging across crops
        """
        if self.num_crops <= 1:
            return inputs
        else:
            B = inputs.shape[0] // self.num_crops
            out = tu.reshape_dimensions(inputs, 0, 0, target_dims=(B, self.num_crops))
    ...
    def __repr__(self):
        return (
            ...
            f"num_crops={self.num_crops}, pos_enc={self.pos_enc})"
        )
