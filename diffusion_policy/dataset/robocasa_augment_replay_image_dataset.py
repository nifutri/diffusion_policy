...
class InMemoryVideoDataset(Dataset):
    ...
    def get_dataset_file(self, task_list):
        ...
        for task in list(SINGLE_STAGE_TASK_DATASETS):
            ...
            # human_path = human_path.replace('/proj/vondrick3/junbang/simulations/packages/robocasa/datasets',
            #                                 '/local/vondrick/junbang/simulations/packages/robocasa/datasets')
    ...
    def __getitem__(self, i):
        ...
        try:
            ...
            relative_actions_abs = self.datasets[task_index]['data'][demo_key]['actions'][demo_step:demo_step+self.pred_horizon*self.stride:self.stride][:, 0:self.action_dim]
            ...
            left_image = self.datasets[task_index]['data'][demo_key]['obs']['robot0_agentview_left_image'][indexed_start:end:self.stride]
            right_image = self.datasets[task_index]['data'][demo_key]['obs']['robot0_agentview_right_image'][indexed_start:end:self.stride]
            gripper_image = self.datasets[task_index]['data'][demo_key]['obs']['robot0_eye_in_hand_image'][indexed_start:end:self.stride]
