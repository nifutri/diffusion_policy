...
def _convert_robocasa_to_replay(store, shape_meta, dataset_path, abs_action, rotation_transformer, 
        n_workers=None, max_inflight_tasks=None):
    if n_workers is None:
        n_workers = multiprocessing.cpu_count()
    if max_inflight_tasks is None:
        # max_inflight_tasks = n_workers * 5
        max_inflight_tasks = 10000
    ...
    with h5py.File(dataset_path) as file:
        ...
        for demo_key in list(demos.keys()):
            ...
            demo_key_to_episode_idx[demo_key] = len(episode_ends)
        ...
        with tqdm(total=n_steps*len(rgb_keys), desc="Loading image data", mininterval=1.0) as pbar:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=n_workers) as executor:
                ...
                for key_idx in range(len(rgb_keys)):
                    ...
                    for demo_key in list(demos.keys()):
                        demo = demos[demo_key]
                        print(f'Loading {key} from {demo_key}')
                        # pdb.set_trace()
                        ...
                        for hdf5_idx in range(hdf5_arr.shape[0]):
                            ...
                            episode_idx = demo_key_to_episode_idx[demo_key]
