...
class SmallTransformNetwork(nn.Module):
    def __init__(self, input_dim, output_dim):
        super().__init__()
        self.body = nn.Sequential(
...
class DiffusionUnetTimmPolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        self.small_transform_network = SmallTransformNetwork(self.action_dim*self.action_horizon, self.action_dim*self.action_horizon)
    ...
    def evaluate_action(self, obs_dict: Dict[str, torch.Tensor], fixed_action_prefix: torch.Tensor=None, actions=None) -> Dict[str, torch.Tensor]:
        ...
        pred_prepared = pred.reshape(batch_dim, -1)
    ...
    def compute_loss(self, batch0, batch1):
        ...
        # now add the stuff for the diffusion policy:
        trajectory_dp = nactions0.clone()
        # Sample noise that we'll add to the images
        noise = torch.randn(trajectory_dp.shape, device=trajectory_dp.device)
        # input perturbation by adding additonal noise to alleviate exposure bias
        # reference: https://github.com/forever208/DDPM-IP
        noise_new = noise + self.input_pertub * torch.randn(trajectory_dp.shape, device=trajectory_dp.device)

        # Sample a random timestep for each image
        timesteps_dp = torch.randint(
            0, self.noise_scheduler.config.num_train_timesteps, 
            (nactions0.shape[0],), device=trajectory_dp.device
        ).long()

        # Add noise to the clean images according to the noise magnitude at each timestep
        # (this is the forward diffusion process)
        noisy_trajectory_dp = self.noise_scheduler.add_noise(
            trajectory_dp, noise_new, timesteps_dp)
        ...
        # COmbine them:
        global_cond_combined = torch.cat((global_cond0, global_cond0, global_cond1), dim=0)
        nactions_combined = torch.cat((noisy_trajectory_dp, nactions0, nactions1), dim=0)
        batch_dim = nactions_combined.shape[0]
        ind_batch = int(batch_dim/3)
        ...
        # Timesteps - here also keep them fixed! - to 0! - 0 is also the timestep of the last denoising!!!
        # Sample a random timestep for each image
        timesteps = torch.randint(
            0, self.noise_scheduler.config.num_train_timesteps, 
            (ind_batch*2,), device=trajectory.device
        ).long()*0
        ...
        pred = self.model(
            trajectory,
            torch.cat((timesteps_dp, timesteps),dim=0), 
            local_cond=None,
            global_cond=global_cond_combined
        )

        pred_dp = pred[:ind_batch,...]
        pred_prepared = pred[ind_batch:,...]

        # now basically reshape and pass thhrough the final layer:
        pred_prepared = pred_prepared.reshape(2*ind_batch, -1)

        f_expert = pred_prepared[:ind_batch,...]
        f_suboptimal = pred_prepared[ind_batch:,...]

        score_expert = self.reward_discriminator(self.small_transform_network(f_expert))
        score_suboptimal = self.reward_discriminator(self.small_transform_network(f_suboptimal))
        ...
        # now add the dp loss on top:
        pred_type = self.noise_scheduler.config.prediction_type 
        if pred_type == 'epsilon':
            target = noise
        elif pred_type == 'sample':
            target = trajectory[:ind_batch,...]
        else:
            raise ValueError(f"Unsupported prediction type {pred_type}")

        loss_dp = F.mse_loss(pred_dp, target, reduction='none')
        loss_dp = loss_dp.type(loss_dp.dtype)
        loss_dp = reduce(loss_dp, 'b ... -> b (...)', 'mean')
        loss_dp = loss_dp.mean()
        print ("loss_dp:", loss_dp.item())
