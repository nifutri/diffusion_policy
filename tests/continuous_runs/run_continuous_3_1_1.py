...
initial_failure_file = f'{home_path}/Code_robocasa/robocasa_dagger/diffusion_policy/data/outputs/ST_OOD_DAgger_train_diffusion_unet_clip_CoffeeServeMug/eval_base_policy/merged_demos_fails_im256.hdf5'
...
proposal_policy_path = f'{home_path}/Code_robocasa/robocasa_dagger/diffusion_policy/data/outputs/PreTrainedModels/CoffeeServeMug/latest.ckpt'
...
for i in range(num_epochs_iterative_procedure):
    ...
    if initialization:
        cmd += f" blending_policy.greedy_behavior=1 blending_policy.blending_horizon=8"
    else:
        cmd += f" blending_policy.greedy_behavior=0 blending_policy.blending_horizon=8"
    ...
    cmd = f"cd {robocasa_directory}/robocasa/scripts && MUJOCO_GL=osmesa python original_robocasa_dataset_states_to_obs.py --dataset {curr_folder_tmp}/merged_demos_fails.hdf5"
    os.system(cmd)
